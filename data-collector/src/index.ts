...
export class KuCoinDataCollector {
  private readonly baseUrl = 'https://api.kucoin.com';
  private readonly wsUrl = 'wss://ws-api.kucoin.com/endpoint';
  ...
  async getMarketData(): Promise<MarketData[]> {
    try {
      ...
      const response = await axios.get(`${this.baseUrl}/api/v1/market/allTickers`);
      ...
      const filteredTickers = tickers
        ...
        .sort((a: MarketData, b: MarketData) => (b.marketCap || 0) - (a.marketCap || 0)) // En yüksek volume'dan başla
        .slice(0, 50); // İlk 50'yi al

      logger.info(`✅ ${filteredTickers.length} yüksek hacimli coin filtrelendi`);
      
      // İlk 5'i logla
      filteredTickers.slice(0, 5).forEach((ticker: MarketData, index: number) => {
  ...
  async getKlineData(
    symbol: string, 
    interval: '15min' | '1hour' | '4hour' | '1day' = '15min',
    limit: number = 200
  ): Promise<KlineData[]> {
    try {
      ...
      const klines = rawKlines
        ...
        .sort((a: KlineData, b: KlineData) => a.openTime - b.openTime); // Zaman sırasına göre
   ...
   */
  private convertToKuCoinInterval(interval: string): string {
  ...
  private async initializeWebSocket(): Promise<void> {
    try {
      ...
      const tokenResponse = await axios.post(`${this.baseUrl}/api/v1/bullet-public`);
      ...
      const server = instanceServers.sort((a: any, b: any) => a.pingInterval - b.pingInterval)[0];
   ...
   */
  private processTickerUpdate(tickerData: any): void {
   ...
   */
  private processTradeUpdate(tradeData: any): void {
    if (!tradeData) return;
   ...
   * Pump/Dump sebebini analiz et
   */
  private async analyzePumpDumpReason(symbol: string, changePercent: number, volume: number, currentPrice: number): Promise<void> {
    try {
